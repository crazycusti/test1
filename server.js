...
const { initDatabase, createTicket, findTicketByUid, listTickets, updateTicketStatus } = require('./src/db');
...
app.post('/tickets', (req, res) => {
  ...
  const { uid } = createTicket({ customerName, subject, note, startAt, endAt });
  res.send(`<!DOCTYPE html>
  <html lang="de">
    <head>
      <meta charset="utf-8" />
...
app.post('/status', (req, res) => {
  ...
  const ticket = findTicketByUid(sanitizedUid);
  if (!ticket) {
